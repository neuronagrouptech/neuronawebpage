    ...
    return (
        <motion.div
            className={`relative transition-all duration-500 ease-in-out cursor-pointer bg-darkLeft rounded-xl
                 ${isExpanded ? 'w-full md:w-[720px] min-h-[460px] z-20' : 'w-full md:w-[580px] min-h-[420px] z-10'}
            ...
            >
                <div className="flex flex-col md:flex-row justify-between items-center mb-4 md:mb-6">
                    <motion.div
                        ...
                        ${isExpanded ? 'w-[100px] h-[100px] md:w-[150px] md:h-[150px]' : 'w-[120px] h-[120px] md:w-[200px] md:h-[200px]'}`}
                        layout
                        initial={{rotate: -15}}
                        animate={{rotate: 0}}
                        transition={{duration: 0.4, ease: "easeOut"}}
...
const Services = () => {
    ...
    const services = [
        {
            icon: <Brain className="w-48 h-48 text-whiteNeurona"/>, service: {
                title: "AI & Data Studio",
                subtitle: "Gen AI & Machine Learning Projects",
                details: ["Cut Costs by eliminating the need for in-house tech teams.", "Scalable Solutions that adapt to your business needs.", "Expert Access to AI and data science without additional training."]
            }
        }, {
            icon: <Lightbulb className="w-48 h-48 text-whiteNeurona"/>, service: {
                title: "Innovation & Product Studio",
                subtitle: "Growth Innovation Partner as a Service (GIPaaS)",
                details: ["Strategic Guidance to uncover growth and expansion opportunities.", "Innovation Enablement with new tech, processes, and products.", "Ongoing Support as a long-term partner, helping you stay ahead of the curve."]
            }
        }, {
            icon: <Code className="w-48 h-48 text-whiteNeurona"/>, service: {
                title: "Software Development Studio",
                subtitle: "Software Engineer as a Service (SEaaS)",
                details: ["Flexible Engagement with no long-term commitments.", "Speed & Efficiency to quickly onboard talent and start projects.", "Global Talent Pool to find the best engineers for your needs."]
            }
        },{
    ...
    useEffect(() => {
        if (carouselRef.current && expandedIndex !== null) {
            ...
            const scrollPosition = card.offsetLeft - (carouselRef.current.offsetWidth - card.offsetWidth) / 2;
            carouselRef.current.scrollTo({
                left: scrollPosition, behavior: "smooth",
            });
        }
    }, [expandedIndex]);
    ...
    return (
        <section id="services" ref={sectionRef}
                 className={`md:p-12 transition-all duration-1000 ease-in-out ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-10"}`}>
            <div className="max-w-[1300px] mx-auto">
                ...
                >
                    <div className="flex flex-col md:flex-row gap-6 py-12 w-full">
                        {services.map((service, index) => (<div key={index} className="snap-start mx-auto md:mx-0">
                            <ServiceCard
                                icon={service.icon}
